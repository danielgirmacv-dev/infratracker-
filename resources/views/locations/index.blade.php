...
@section('header')
    <div class="breadcrumb">
        ...
        <span class="text-slate-500 dark:text-slate-400">Add Location</span>
    </div>
@endsection
...
<div class="space-y-6 animate-slide-up" x-data="{ deleteModalOpen: false, deleteUrl: '', deleteLabel: '' }">
    ...
    @if(request()->filled('search'))
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-brand-500/20 bg-brand-500/5 px-4 py-3 text-sm">
            ...
            <a href="{{ route('locations.index') }}" class="text-xs font-semibold text-brand-600 dark:text-brand-400 hover:underline">Clear filter</a>
        </div>
    @endif
    ...
    <div class="flex flex-col gap-6 lg:flex-row">
        ...
        <div class="w-full lg:w-1/3 space-y-6">
            <div class="card p-6 border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                <h2 class="text-base font-bold text-slate-800 dark:text-slate-200 mb-4">Register New Location</h2>
                
                <form action="{{ route('locations.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="name" class="form-label">Location Name <span class="text-red-600">*</span></label>
            ...
            <div class="card p-6 border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">Upload an Excel or CSV file with a column header: <code class="bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-[11px] font-mono">location_name</code> or <code class="bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-[11px] font-mono">name</code></p>
                
                <form action="{{ route('locations.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
        ...
        <div class="w-full lg:w-2/3">
            <div class="card overflow-hidden border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                <div class="border-b border-slate-200 dark:border-white/5 px-6 py-4 flex items-center justify-between">
                    <h2 class="text-base font-bold text-slate-800 dark:text-slate-200">Registered Locations</h2>
                    <span class="rounded-full bg-indigo-100 dark:bg-indigo-500/20 px-2 py-0.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400">{{ $locations->count() }} Total</span>
                </div>
                ...
                <div class="table-scroll-wrap">
                    <table class="data-table w-full min-w-[28rem]">
                        ...
                        <tbody>
                            @forelse ($locations as $idx => $location)
                                <tr>
                                    <td class="font-mono text-xs text-slate-400 dark:text-slate-500">{{ $idx + 1 }}</td>
                                    <td class="font-medium text-slate-800 dark:text-slate-200">{{ $location->name }}</td>
                                    <td class="text-slate-500 dark:text-slate-400 text-xs">{{ $location->created_at->format('d M Y, h:i A') }}</td>
                                    <td class="text-right">
                                        <button
                                            type="button"
                                            class="inline-flex rounded-lg p-1.5 text-red-600 dark:text-red-400 transition hover:bg-red-100 dark:hover:bg-red-500/10"
                                            title="Delete"
                                            @click="deleteModalOpen=true; deleteUrl=@js(route('locations.destroy', $location)); deleteLabel=@js($location->name)"
                                        >
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-12 text-center text-slate-500 dark:text-slate-400">
                                        No locations registered yet. Add one manually or import from Excel.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
