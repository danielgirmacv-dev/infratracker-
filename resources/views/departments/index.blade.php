...
@section('header')
    <div class="breadcrumb">
        ...
        <span class="text-slate-500 dark:text-slate-400">Add Department</span>
    </div>
@endsection
...
<div class="space-y-6 animate-slide-up" x-data="{ deleteModalOpen: false, deleteUrl: '', deleteLabel: '' }">
    ...
    @if(request()->filled('search'))
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-brand-500/20 bg-brand-500/5 px-4 py-3 text-sm">
            ...
            <a href="{{ route('departments.index') }}" class="text-xs font-semibold text-brand-600 dark:text-brand-400 hover:underline">Clear filter</a>
        </div>
    @endif
    ...
    <div class="flex flex-col gap-6 lg:flex-row">
        ...
        <div class="w-full lg:w-1/3 space-y-6">
            <div class="card p-6 border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                <h2 class="text-base font-bold text-slate-800 dark:text-slate-200 mb-4">Register New Department</h2>
                
                <form action="{{ route('departments.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="name" class="form-label">Department Name <span class="text-red-600">*</span></label>
            ...
            <div class="card p-6 border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">Upload an Excel or CSV file with a column header: <code class="bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-[11px] font-mono">department_name</code> or <code class="bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-[11px] font-mono">name</code></p>
        ...
        <div class="w-full lg:w-2/3">
            <div class="card overflow-hidden border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                <div class="border-b border-slate-200 dark:border-white/5 px-6 py-4 flex items-center justify-between">
                    <h2 class="text-base font-bold text-slate-800 dark:text-slate-200">Registered Departments</h2>
                    <span class="rounded-full bg-indigo-100 dark:bg-indigo-500/20 px-2 py-0.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400">{{ $departments->count() }} Total</span>
                </div>
                        ...
                        <tbody>
                            @forelse ($departments as $idx => $department)
                                <tr>
                                    ...
                                    <td class="font-medium text-slate-800 dark:text-slate-200">{{ $department->name }}</td>
                                    <td class="text-slate-500 dark:text-slate-400 text-xs">{{ $department->created_at->format('d M Y, h:i A') }}</td>
                                    <td class="text-right">
                                        <button
                                            type="button"
                                            class="inline-flex rounded-lg p-1.5 text-red-600 dark:text-red-400 transition hover:bg-red-100 dark:hover:bg-red-500/10"
                                            title="Delete"
                                            @click="deleteModalOpen=true; deleteUrl=@js(route('departments.destroy', $department)); deleteLabel=@js($department->name)"
                                        >
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-12 text-center text-slate-500 dark:text-slate-400">
                                        No departments registered yet. Add one manually or import from Excel.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
